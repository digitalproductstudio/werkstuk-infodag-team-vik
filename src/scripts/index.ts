import { GestureRecognizer, GestureRecognizerOptions, FilesetResolver } from "@mediapipe/tasks-vision";
            ...
            detectThumbsUp(results).then(isThumbsUp => {
                if (isThumbsUp) {
                    ...
                    if (thumbsUpTimer) {
                        ...
                        thumbsUpTimer = null; // Zet de timer terug naar null
                    }
...
function drawLandmarks(landmarks: Landmark[], ctx: CanvasRenderingContext2D): void {
    const palmRadius = 2;
...
function drawHandLines(landmarks: Landmark[], ctx: CanvasRenderingContext2D): void {
    const connections = [
...
async function detectThumbsUp(results: HandsResult): Promise<boolean> {
    ...
    const detections = await gestureRecognizer.recognizeForVideo(videoElement, performance.now());
    ...
    const thumbsUpGesture = detections.gestures.some(gestureSet =>
        gestureSet.some(gesture => gesture.categoryName === "Thumb_Up" && gesture.score > 0.6)
    );
